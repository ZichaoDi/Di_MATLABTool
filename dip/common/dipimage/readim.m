...
% 30 January 2015:  Avoiding errors with wrong metadata while reading BioFormats (Rainer Heintzmann)
...
p = javaclasspath('-all');
p = regexp(p,'(loci_tools\.jar|bioformats_package\.jar)$','once');
p = [p{:}];
if isempty(p)
...
for imageseries = 0:nSeries-1
   ...
   try
      ...
      warning('Bioformats reader: Problem loading metadata. Skipping');
      metadata={};
   end
