...
% 28 November 2014: Better display of a single value (Piet Verbeek's idea).
...
if isempty(in1.data)
   ...
   if prod(v) == 1
      in = double(in);
      if ndims(in)<=2 % this doesn't work for higher dimensions.
         disp(['        value ',mat2str(in,5)]);
      end
   end
end
