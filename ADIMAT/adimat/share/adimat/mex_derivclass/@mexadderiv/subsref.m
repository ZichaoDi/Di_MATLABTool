...
%MEXADDERIV/SUBSREF Index into to derivative object
%
...
case '{}'
   if g.dims==1
      ...
      elseif max(ind{1})> g.ndd(1)
         ...
         index{1}= repmat(ind{1}, 1, g.ndd(1))+ reshape(...
      ...
      elseif max(ind{2})>g.ndd(2)
         ...
         index{2}= repmat(ind{2}, 1, g.ndd(2))+ reshape(...
...
case '.'
   ...
   %varargout{1}= res;
   error('mexadderiv/subsref for structure: Not supported, and should not need to be supported.');
otherwise
