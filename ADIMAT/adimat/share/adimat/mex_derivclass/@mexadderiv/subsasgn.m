...
%MEXADDERIV/SUBSASGN Assign a value to the derivative object.
%
...
case '{}'
   if g.dims==1
      ...
      if size(index(1).subs,2)==2
         if all([index(1).subs{1:2}]<= g.ndd') % [g.ndd(:)]'
            if size(index, 2)> 1
...
case '()'
   ...
   if deletemode
      ...
      inp= mexadderiv(g.ndd, [], 'empty');
   elseif ~isa(inp, 'mexadderiv')
