function g= mexadderiv(n,sz,mode)
...
%
% g= mexadderiv(ndd, sz, 'zeros') Creates a new derivative object with storage
...
%     may be omited.
%
% g= mexadderiv(ndd, sz, 'cond_sparse') The same as using 'zeros' but this time
%     a cond_sparse datastructure is used.
%
% g= mexadderiv(ndd, sz, 'object') Uses copies of the object sz to stored in
%     every directional derivative.
%
% g= mexadderiv(ndd, sz, 'empty') Does not care about the data in sz. I.e.,
...
if nargin== 0
   error('An mexadderiv object needs to know its number of directional derivatives.');
elseif nargin==1 && isa(n, 'mexadderiv')
   g= struct('deriv', [], 'dims', n.dims, 'ndd', n.ndd, 'sz', n.sz);
   g= class(g, 'mexadderiv');
   return
elseif isstruct(n) && nargin==1 % && isequal(fieldnames(n), {'deriv'; 'dims'; 'ndd'; 'sz'})
      g= class(n, 'mexadderiv');
      return;
else
   if isempty(n)
   ...
   if strcmp(mode,'zeros') || strcmp(mode,'sparse')
      switch size(sz, 2)
         case 1
            g.deriv= zeros(sz*g.ndd(1), sz*g.ndd(2));
            g.sz= [sz, sz];
         case 2
            g.deriv= zeros(sz.*g.ndd);
            g.sz= sz;
         otherwise
            error('Sparse matrizes have a maximum of two dimensions. Current number of dimensions: %d', length(sz));
         end
   else
      switch mode
         case 'object'
