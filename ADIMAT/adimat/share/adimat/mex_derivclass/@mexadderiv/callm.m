...
%MEXADDERIV/CALLM Call func with all arguments are deriv. objs. and varargout
%
...
% modification and distribution FORBIDDEN!
   ...
   resn= nargout;
   [varargout{1:resn}]= deal(mexadderiv(varargin{1}));
   ...
   for c= 1: resn
      if ndims(outcell{c})>2
         error('The mexadderiv-class supports only 2D objects for directional derivatives. Current number of dimension is: %d.', ndims(outcell{c}));
      end
      sz(c,:)= size(outcell{c});
      varargout{c}.sz= sz(c,:);
