...
function obj = binop(obj, right, handle)
  if isa(obj, 'foderiv')
    if isa(right, 'foderiv')
      obj.m_derivs = cellfun(handle, obj.m_derivs, right.m_derivs, 'UniformOutput', false);
    else
      obj.m_derivs = cellfun(@(x) handle(x, right), obj.m_derivs, 'UniformOutput', false);
    end
  else
    val = obj;
    obj = right;
    obj.m_derivs = cellfun(@(y) handle(val, y), right.m_derivs, 'UniformOutput', false);
  end
end
