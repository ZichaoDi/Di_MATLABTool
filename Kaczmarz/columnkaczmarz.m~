function [X info] = columnkaczmarz(A,b,K,x0,options)
global ws N
ws=x0;
%COLUMNKACZMARZ Column Kaczmarz method for Tikhonov regularization problem
%
...
%
% where j(k) = (k-1) mod n + 1, k = 1,2, ... ., and a^j(k) - column of matrix
...
%   [x1 info1] = columnkaczmarz(A,b,N,[],options);
%
...
% [6]   Ivanov A.A., Zhdanov A.I. Kaczmarz algorithm for Tikhonov regularization problem,
...
k = 0;
l = 0;
fprintf('iter      residule        error\n')
while ~stop
    k = k + 1;
    sing_rec(k,:)=[k-1,norm(b-A*xk),norm(ws-xk)];
    fprintf('%d      %e      %e\n',sing_rec(k,:));
    xkm1 = xk;
    for i = I
        % The iterate.
        ...
        ro = (ari*rk - (alpha)*xk(i))/(normAi(i) + alpha);
        rk = rk - ro*ari';
        xk(i) = xk(i) + ro;
        
        if nonneg && xk(i) < 0, xk(i) = 0; end
    end
    xk1 = xk;
